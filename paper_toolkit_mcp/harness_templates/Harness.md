...
**违规示例**：
- [X] `P=0.030`（P≥0.01应保留2位小数，即P=0.03）
- [X] `P=0.0060`（0.001≤P<0.01应保留3位小数，即P=0.006）
- [X] `p<0.001`（应大写P）
- [X] `P=0.0001`（应报告为P<0.001）
...
**违规示例**：
- [X] `| 96.0% | 85.5% |`（%应提取到表头，单元格只写数值）
- [X] `96%`（率应保留1位小数，即96.0%）
- [X] `85.500%`（百分率不应保留3位以上小数）

**正确示例**：
- [OK] 表头写"数值（%）"，单元格写"96.0""85.5"
- [OK] 正文中独立出现的百分率写"96.0%"（保留1位小数）
...
| R6.1 引用位置 | A | 引用标记不在标点前 | 调整引用位置 |
| R6.2 多文献同引 | A | 多文献引用格式错误 | 使用分号分隔 |
| R6.3 分号与引用对应 | A | 分号数量与引用不匹配 | 核对分号与引用 |
| R7.1 标题冒号 | A | "消融实验：XXX" | "XXX的消融实验" |
| R7.2 自我夸大 | A | "极其创新" | 客观表述 |
| R7.3 回引 | A | "见本文2.3.1节" | 删除回引 |
| R8.6 用户意见 | A | 【】标记残留 | 处理意见 |
