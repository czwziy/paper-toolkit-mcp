...
class UnpaywallResolver:
    ...
    def __init__(self, email: str | None = None):
        ...
        if not self.email:
            logger.warning(
                "paper_toolkit_mcp_UNPAYWALL_EMAIL/UNPAYWALL_EMAIL not configured. "
        ...
        try:
            ...
            if response.status_code == 422:
                logger.warning(
                    ...
                    "Please use a valid contact email in paper_toolkit_mcp_UNPAYWALL_EMAIL/UNPAYWALL_EMAIL.",
                    self.email,
                )
                return None
...
class UnpaywallSearcher(PaperSource):
    ...
    def search(self, query: str, max_results: int = 10, **kwargs) -> list[Paper]:
        ...
        if not self.resolver.has_api_access():
            logger.warning(
                "Unpaywall search skipped: missing paper_toolkit_mcp_UNPAYWALL_EMAIL/UNPAYWALL_EMAIL."
            )
            return []
    ...
    def download_pdf(self, paper_id: str, save_path: str = "./downloads") -> str:
        raise NotImplementedError(
            ...
            "Use the returned pdf_url/url with download_paper or source-native download tools."
        )
