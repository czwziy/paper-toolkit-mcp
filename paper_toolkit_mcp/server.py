...
# All search results are upserted here; download_with_fallback checks here first.
...
def _dedupe_papers(papers: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    upserted into the SQLite storage for cross-session lookup.
    """
...
async def download_with_fallback(
    ...
    primary_downloaders = {
        ...
        "dblp": dblp_searcher.download_pdf,
        "openalex": openalex_searcher.download_pdf,
    }
    ...
    if source_name in primary_downloaders:
        try:
            ...
            if isinstance(primary_result, str) and os.path.exists(primary_result):
                storage.set_local_pdf(dedup_key, primary_result)
    ...
    if repository_result:
        storage.set_local_pdf(dedup_key, repository_result)
    ...
    if normalized_doi:
        ...
        if unpaywall_url:
            ...
            if unpaywall_result:
                storage.set_local_pdf(dedup_key, unpaywall_result)
    ...
    if fallback_result:
        storage.set_local_pdf(dedup_key, fallback_result)
...
async def library_search(
    ...
    """
    return storage.search_local(keyword, limit=limit)
